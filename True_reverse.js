...
let exchange = (arr, index1, index2) => {// запятая лишняя,но не критично
    let buffer = arr[1]; // почему 0? мы ж меняем местами arr[index1] и arr[index2].
    arr[1] = arr[5]; // вот этим шагом м ы теряем значение 5 го элемента, с концами. и теперь у нас значение из 0 вого элемента хранится в буфере, под 0вым индексом и под 5ым индексом
    return arr() // не понял маневра - это зачем?
}
